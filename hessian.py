...
class Hessian(object):
    def __init__(self, molecule, inp_file_obj, options, path="."):
        ...
        self.singlepoints = []
        psi_mol_obj = self.molecule.cast_to_psi4_molecule_object()
        for disp_num, disp in enumerate(
                psi4.driver_findif.hessian_from_energy_geometries(
                    psi_mol_obj, -1)):
    ...
    def compute_hessian(self, sow=True):
        ...
        self.hessian = psi4.driver_findif.compute_hessian_from_energy(
            psi4_mol_obj, self.energies, -1)
