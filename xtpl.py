...
class Xtpl(Procedure):
    ...
    def get_result_1_4(self, force_resub=False):
    ...
    def get_result_1_6(self, force_resub=False):
        ...
        corr_result = psi4.driver_cbs_helper.corl_xtpl_helgaker_2(f"{self.job_type}",
                                                         zLO=self.options.xtpl_basis_sets[0][1],
                                                         valueLO=results[1].to_array(),
                                                         zHI=self.options.xtpl_basis_sets[0][0],
                                                         valueHI=results[0].to_array())
        corr_energy = psi4.driver_cbs_helper.corl_xtpl_helgaker_2("energies",
                                                         zLO=self.options.xtpl_basis_sets[0][1],
                                                         valueLO=energies[1],
                                                         zHI=self.options.xtpl_basis_sets[0][0],
                                                         valueHI=energies[0])
        ...
        # a dz, tz, qz scf but only need the qz and tz in compensating for extrapolating the
        # reference energy with the correlation energy
        scf_result_corr = psi4.driver_cbs_helper.corl_xtpl_helgaker_2(f"scf correlated {self.job_type}",
                                                             zLO=self.options.xtpl_basis_sets[0][1],
                                                             valueLO=results[3].to_array(),
                                                             zHI=self.options.xtpl_basis_sets[0][0],
                                                             valueHI=results[2].to_array())
        scf_energy_corr = psi4.driver_cbs_helper.corl_xtpl_helgaker_2("scf correlated energies",
                                                             zLO=self.options.xtpl_basis_sets[0][1],
                                                             valueLO=energies[3],
                                                             zHI=self.options.xtpl_basis_sets[0][0],
                                                             valueHI=energies[2])
        ...
        if self.options.scf_xtpl:
            ...
            if len(self.procedure_options[-1]) == 5:
                scf_result = psi4.driver_cbs_helper.scf_xtpl_helgaker_3(f"{self.job_type}",
                                                               zLO=self.options.xtpl_basis_sets[1][-1],
                                                               valueLO=results[-1].to_array(),
                                                               zMD=self.options.xtpl_basis_sets[1][-2],
                                                               valueMD=results[-2].to_array(),
                                                               zHI=self.options.xtpl_basis_sets[1][-3],
                                                               valueHI=results[-3].to_array())
                scf_energy = psi4.driver_cbs_helper.scf_xtpl_helgaker_3("energies",
                                                               zLO=self.options.xtpl_basis_sets[1][-1],
                                                               valueLO=energies[-1],
                                                               zMD=self.options.xtpl_basis_sets[1][-2],
                                                               valueMD=energies[-2],
                ...
                scf_result = psi4.driver_cbs_helper.scf_xtpl_helgaker_2(f"{self.job_type}",
                                                               zLO=self.options.xtpl_basis_sets[1][-1],
                                                               valueLO=results[-1].to_array(),
                                                               zHI=self.options.xtpl_basis_sets[1][-2],
                                                               valueHI=results[-2].to_array())
                scf_energy = psi4.driver_cbs_helper.scf_xtpl_helgaker_2("eneriges",
                                                               zLO=self.options.xtpl_basis_sets[1][-1],
                                                               valueLO=energies[-1],
                                                               zHI=self.options.xtpl_basis_sets[1][-2],
                                                               valueHI=energies[-2])
            ...
            # don't extrapolate add to largest scf
            scf_result = psi4.driver_cbs_helper.xtpl_highest_1(f"{self.job_type}",
                                                       zHI=self.options.xtpl_basis_sets[1][-2],
                                                       valueHI=results[-2].to_array())

            scf_energy = psi4.driver_cbs_helper.xtpl_highest_1("eneriges",
                                                       zHI=self.options.xtpl_basis_sets[1][-2],
                                                       valueHI=energies[-2])
    ...
    def get_result(self, force_resub=False):
        ...
        if psi_version < 1.6:
            self.get_result_1_4(force_resub)
        else:
            self.get_result_1_6(force_resub)
